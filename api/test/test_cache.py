...
class CacheTesting(TestCase):  # django is creating new DB
    def test_add_get_cache_book(self):
        ...
        book_cache.set_object(object_id=book_1.id, model_object=book_1)
    ...
    def test_invalidate_cache(self):
        ...
        book_cache.set_object(object_id=book_1.id, model_object=book_1)
        book_cache.set_object(object_id=book_2.id, model_object=book_2)
    ...
    def test_spam_cache_book(self):
        ...
        book_cache.set_object(object_id=books[0].id, model_object=books[0])
        book_cache.set_object(object_id=books[1].id, model_object=books[1])
    ...
    def test_concurrent_access(self):
        def get_book_from_cache(book_id: int, result_holder: list) -> Optional[Book]:
            result = book_cache.get_object(book_id)
            result_holder[0] = result
            return result

        def delete_book_from_cache(book_id: int) -> None:
            book_cache.remove_object(book_id)
