...
class Cache:
    ...
    def __init__(self, size: int = 3, default_ttl: int = 60):
        ...
        self.cache_liked_list = BidirectionalLinkedList()
        self.lock = threading.Lock()

    def get_object(self, object_id: int) -> Optional[Any]:
        if object_id in self.cache_liked_list.node_dict:
            with self.lock:
                # re-entering node to renew its TTL
                object_access_time = time.time()
                curr_node = self.cache_liked_list.get_node(object_id)
                self.cache_liked_list.remove_node(object_id)
                model_object = curr_node.value
                self.cache_liked_list.add_node(model_object, object_access_time, model_object.id)
    ...
    def set_object(self, object_id: int, model_object: Any) -> None:
        with self.lock:
            object_access_time = time.time()
            if object_id not in self.cache_liked_list.node_dict:
                self.__ensure_cache_size()
                self.cache_liked_list.add_node(model_object, object_access_time, object_id)

        self.__create_delete_timer(object_id, object_access_time)

    def remove_object(self, object_id: int, periodic_delete_time: float = None) -> None:
        in_cache = object_id in self.cache_liked_list.node_dict
        with self.lock:
    ...
    def __ensure_cache_size(self) -> None:
        if len(self.cache_liked_list.node_dict) >= self.size:
            oldest_node = self.cache_liked_list.get_head()
